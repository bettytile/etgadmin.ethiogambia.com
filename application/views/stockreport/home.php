  ...
  function fetch_data(){
    ...
    $i = 0;
          if (isset($_POST['find'])) {
                ...
                // if ($p_code=="all") {
                //   $sel = "SELECT * FROM stock,product,station WHERE product.prod_id = stock.id_product AND station.s_id = stock.id_station  
                //       AND id_station='{$station}' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                ...
                // }
                  if ($station=="all") {
                    $sel = "SELECT * FROM stock,station WHERE station.s_id = stock.id_station AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                                  $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                        while ($row=mysqli_fetch_array($run)) {
                  ...
                  elseif ($station=="all" && $p_code=="all") {
                    $sel = "SELECT * FROM stock,station WHERE station.s_id = stock.id_station  
                    ...
                    $sel = "SELECT * FROM stock,station WHERE station.s_id = stock.id_station AND id_station='{$station}' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                        $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                        while ($row=mysqli_fetch_array($run)) {
