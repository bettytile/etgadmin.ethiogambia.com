    ...
    $i = 0;
          if (isset($_POST['find'])) {
            ...
            $fdate=$_POST['from'];
                  ...
                  $sel="";
                  if($customer == "all"){
                  $sel = "SELECT * FROM delivery,station,customers WHERE station.s_id=delivery.id_station AND customers.customer_id = delivery.id_customer AND id_station='{$station}'
                  ...
                  elseif ($station == "all") {
                    $sel = "SELECT * FROM delivery,station,customers WHERE  station.s_id=delivery.id_station AND customers.customer_id = delivery.id_customer AND id_customer='{$customer}'
                  AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                  $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                      while ($row=mysqli_fetch_array($run)) {
                                $outputs .='<tr>
                    ...
                    $sel = "SELECT * FROM delivery,station,customers WHERE  station.s_id=delivery.id_station AND customers.customer_id = delivery.id_customer AND id_customer='{$customer}' AND id_station='{$station}'
                    AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                    $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                        while ($row=mysqli_fetch_array($run)) {
                                  $outputs .='<tr>
    ...
    <span class="input-group-addon" id="sizing-addon2">
      ...
      <input type="date" class="form-control" placeholder="To Date" name="to" aria-describedby="sizing-addon2">
        ...
        <div class="input-group form-group">
            ...
            <select name="id_customer" class="form-control select2" aria-describedby="sizing-addon2">
