    ...
    $i = 0;
          if (isset($_POST['find'])) {
                  ...
                  if ($product_code == "all") {
                   ...
                   AND machine.mach_id=production.id_machine AND station.s_id=production.id_station AND id_station='{$station}' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                  ...
                  if ($station == "all") {
                    $sel = "SELECT * FROM production,machine,station, production_type WHERE  production_type.production_type_id = production.id_production_type 
                   AND machine.mach_id=production.id_machine AND station.s_id=production.id_station AND id_product='{$product_code}' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                   $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                    ...
                    $sel = "SELECT * FROM production, product,machine,station, production_type WHERE  production_type.production_type_id = production.id_production_type 
                     AND machine.mach_id=production.id_machine AND station.s_id=production.id_station AND id_station='{$station}' AND id_product='{$product_code}' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                     $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
...
<div class="box">
  ...
  <h3 style="display:block; text-align:center;">Product Report</h3>
      ...
      <input type="date" class="form-control" placeholder="To Date" name="to" aria-describedby="sizing-addon2">
        ...
        <div class="input-group form-group">
            ...
            <select name="id_product" class="form-control select2" aria-describedby="sizing-addon2">
                ...
                <?php
                foreach ($dataProduct as $name) {
                ?>
                <option value="<?php echo $name->prod_id; ?>">
                    <?php echo $name->product_code; ?>
                </option>
                <?php
                }
                ?>
