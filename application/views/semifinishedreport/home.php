  ...
  function fetch_data(){
    ...
    $i = 0;
          if (isset($_POST['find'])) {
                ...
                // }
                  if ($station=="all") {
                    $sel = "SELECT * FROM stock_semifinished,preform,station WHERE preform.pre_id = stock_semifinished.raw_material_type AND station.s_id = stock_semifinished.store_location AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                       $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                        while ($row=mysqli_fetch_array($run)) {
                                $outputs .='<tr>
                  ...
                  elseif ($station=="all" && $type=="all") {
                    $sel = "SELECT * FROM stock_semifinished,preform,station WHERE preform.pre_id = stock_semifinished.raw_material_type AND station.s_id = stock_semifinished.store_location  
                    ...
                    $sel = "SELECT * FROM stock_semifinished,preform,station WHERE preform.pre_id = stock_semifinished.raw_material_type AND station.s_id = stock_semifinished.store_location AND store_location='{$station}' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                        $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                        while ($row=mysqli_fetch_array($run)) {
                            $outputs .='<tr>
                            <td>'.$row['preform_code'].'</td>
...
<div class="box">
  ...
  <h3 style="display:block; text-align:center;">stock_semifinished Report</h3>

  <form id="form-stock_semifinished-report" method="POST">
