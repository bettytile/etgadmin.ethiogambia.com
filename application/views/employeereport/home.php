    ...
    $i = 0;
          if (isset($_POST['find'])) {
            ...
            $fdate=$_POST['from'];
                  ...
                  if ($employee == "all") {
                    $sel = "SELECT * FROM production,machine,station, production_type,employees WHERE employees.employee_name=production.id_employee AND  production_type.production_type_id = production.id_production_type 
                  AND machine.mach_id=production.id_machine AND production.id_station=station.s_id AND production.id_station='{$station}' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                  ...
                  elseif ($station == "all") {
                    $sel = "SELECT * FROM production,machine,station, production_type,employees WHERE employees.employee_name=production.id_employee AND  production_type.production_type_id = production.id_production_type 
                   AND machine.mach_id=production.id_machine AND production.id_station=station.s_id AND production.id_employee='".$employee."' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                   $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                    ...
                    $sel = "SELECT * FROM production,machine,station, production_type,employees WHERE production.id_employee=employees.employee_name AND production_type.production_type_id = production.id_production_type 
                     AND machine.mach_id=production.id_machine AND production.id_station=station.s_id AND production.id_station='{$station}' AND id_employee='".$employee."' AND activity_date BETWEEN '".$fdate."' AND '".$tdate."' ORDER BY activity_date DESC"; 
                     $run = mysqli_query($con, $sel) or die("Error: ".mysqli_error($con));
                    
                      while ($row=mysqli_fetch_array($run)> 0) {
...
<div class="box">
  ...
  <h3 style="display:block; text-align:center;">Employee Report</h3>

  <form id="form-product-report" method="POST">
    ...
    <div class="input-group form-group">
      ...
      <input type="date" class="form-control" placeholder="To Date" name="to" aria-describedby="sizing-addon2">
        ...
        <div class="input-group form-group">
            ...
            <select name="id_employee" class="form-control select2" aria-describedby="sizing-addon2">
                <option value="all">All</option>
                <?php
                foreach ($dataEmployees as $name) {
                ?>
                <option value="<?php echo $name->employee_name; ?>">
                    <?php echo $name->employee_name; ?>
                </option>
                <?php
                }
                ?>
