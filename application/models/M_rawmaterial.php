...
class M_rawmaterial extends CI_Model {
	public function select_all() {
		$sql = "SELECT * FROM stock_rawmaterial";
	...
	public function select_all_instock() {
		$sql = " SELECT stock_rawmaterial.rstock_id AS id, stock_rawmaterial.raw_material_type AS raw_material_type, stock_rawmaterial.id_employee AS employee,stock_rawmaterial.received_qty AS received_qty,stock_rawmaterial.issued_qty AS issued_qty, stock_rawmaterial.available_qtys AS available_qty, stock_rawmaterial.reference_no AS reference_no, stock_rawmaterial.id_station AS station, stock_rawmaterial.activity_date AS activity_date, stock_rawmaterial.rstatus AS rstatus,stock_rawmaterial.input_type AS input_type,stock_rawmaterial.warehouse AS warehouse,stock_rawmaterial.store_location AS store_location, raw_material.rawmaterial_code AS code , station.station_name AS station FROM stock_rawmaterial, raw_material, station WHERE  stock_rawmaterial.raw_material_type =raw_material.rm_id AND stock_rawmaterial.store_location=station.s_id AND confirmation_status='0'";
		$data = $this->db->query($sql);
	...
	public function update($data) {
		...
		$sql = "UPDATE stock_rawmaterial SET available_qtys= '".$data['available_qty']."', received_qty= '".$data['received_qty']."', issued_qty= '".$data['issued_qty']."', rstatus= '".$data['rstatus']."', reference_no= '".$data['reference_no']."', activity_date= '".$data['activity_date']."', id_station= '".$data['station']."', raw_material_type= '$id_products' WHERE rstock_id ='" .$data['id'] ."'";
	...
	public function updateStockcopy($id){
        ...
        $query = "UPDATE stock_rawmaterial_copy SET available_qty = '".$row['available_qtys']."'WHERE id_product ='" .$row['raw_material_type'] ."' AND id_station='".$row['store_location']."'";
        $this->db->query($query);
        return $this->db->affected_rows();
	}
	...
	public function insert($data) {
        ...
        if ($row > 0 && $rstatus == "received") {
            $sqlU = "UPDATE stock_rawmaterial SET available_qtys = '".$total."', r_weight = '".$total_weight."' WHERE raw_material_type ='".$rawm_type."'";

		$this->db->query($sqlU);
        }
        elseif ($row > 0 && $rstatus == "issued") {
            $sqlU = "UPDATE stock_rawmaterial SET available_qtys = '".$itotal."', r_weight = '".$iweight."' WHERE raw_material_type ='".$rawm_type."'";
        ...
        $this->db->query($sqlIn);
            ...
            $sqlI = "INSERT INTO stock_rawmaterial (id_employee,raw_material_type,r_weight,available_qty,reference_no,station,activity_date,rstatus) 
		...
		$sql = "UPDATE stock_rawmaterial SET stock_rawmaterial.confirmation_status='1' WHERE stock_rawmaterial.rstock_id='".$id."'";
