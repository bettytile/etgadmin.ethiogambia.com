...
class Confirmation extends AUTH_Controller {
	...
	public function index() {
        ...
        $data['dataConfirmation'] = $this->M_confirmation->select_all();
       // $data['dataRawmaterial'] = $this->M_product->select_all_rawmaterial();
        $data['dataRawmaterials'] = $this->M_rawmaterials->select_all_rawmaterial();
		...
		$this->template->views('confirmation/home', $data);
    }
	public function showData() {
		$data['dataConfirmation'] = $this->M_confirmation->select_all();
		$this->load->view('Confirmation/list_data', $data);
	}
...
// 	}
	...
	public function delete() {
		$id = $_POST['id'];
		$result = $this->M_confirmation->updateStokeRm($id);

		if ($result > 0) {
		    $results = $this->M_confirmation->updateStoke($id);
		    if($results > 0){
		        $resultd = $this->M_confirmation->delete($id);
		        if($resultd > 0){
	...
	public function export() {
		...
		$data = $this->M_confirmation->select_all_inproduction();
		...
		foreach($data as $value){
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $value->code, PHPExcel_Cell_DataType::TYPE_STRING); 
