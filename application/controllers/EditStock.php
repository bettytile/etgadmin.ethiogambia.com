...
class EditStock extends AUTH_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->model('M_stockreport');
	...
	public function index() {
		...
		$data['dataStock'] = $this->M_stockreport->select_all();
		...
		$data['page'] = "editstock";
		$data['judul'] = "Stock Data";
		$data['deskripsi'] = "Manage Stock Data";

		$data['modal_production_report'] = show_my_modal('modals/modal_production_report', 'stock-report', $data);

		$this->template->views('editstock/home', $data);
	}

	public function show() {
		$data['dataStock'] = $this->M_stockreport->select_all();
		$this->load->view('editstock/home', $data);
	}
	...
	// 		$result = $this->M_stockreport->insert($data);
	...
	public function update() {
		...
		$data['dataStock'] = $this->M_stockreport->select_by_id($id);
        $data['dataProductionType'] = $this->M_productionType->select_all_ProductionType();
        $data['dataProduct'] = $this->M_product->select_all_product();
        $data['dataStation'] = $this->M_station->select_all_stations();
		$data['userdata'] = $this->userdata;

		echo show_my_modal('modals/modal_update_stockproduct', 'update-stockproduct', $data);
	}

	public function prosesUpdate() {
		$this->form_validation->set_rules('p_code_n_id', 'Product Code', 'trim|required');
			
		
		$data = $this->input->post();
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_stockreport->update($data);
	...
	public function export() {
		...
		$data = $this->M_stockreport->select_all();
		...
		foreach($data as $value){
		    ...
		    $objPHPExcel->getActiveSheet()->setCellValueExplicit('B'.$rowCount, $value->available_product);
