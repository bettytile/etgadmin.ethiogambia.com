...
class SemiFinishedConfirmation extends AUTH_Controller
{
    ...
    public function index()
    {
        ...
        $data['dataProduct'] = $this->M_preform->select_all_preform();
        ...
        $data['page'] = 'semifinishedconfirmation';
        $data['judul'] = 'Semi Finished Data';
        $data['deskripsi'] = 'Manage Semi Finished Data';

        $data['modal_production_report'] = show_my_modal(
            'modals/modal_production_report',
            'stock-report',
            $data
        );

        $this->template->views('semifinishedconfirmation/home', $data);
    }

    public function show()
    {
        $data['dataSemifinished'] = $this->M_semifinished->select_all_instock();
        $this->load->view('semifinishedconfirmation/home', $data);
    }
    ...
    // 		$result = $this->M_rawmaterial->insert($data);
    ...
    public function update()
    {
        ...
        $data['dataSemifinished'] = $this->M_rawmaterial->select_by_id($id);
        $data['dataProductionType'] = $this->M_productionType->select_all_ProductionType();
        $data['dataStation'] = $this->M_station->select_all_stations();
        $data['userdata'] = $this->userdata;

        echo show_my_modal(
            'modals/modal_update_stockrawmaterial',
            'update-stockRawmaterial',
            $data
        );
    }
    ...
    {
        $id = $_POST['id'];
        $result = $this->M_semifinished->delete($id);
    ...
    public function rmconfirm()
    {
        $id = $_POST['id'];
        $result = $this->M_semifinished->confirmationrm($id);

        if ($result > 0) {
            echo show_succ_msg('Product confirmed!', '20px');
        } else {
            echo show_err_msg('Failed to Confrim Product', '20px');
        }
    }
    ...
    public function export()
    {
        ...
        $objPHPExcel
            ->getActiveSheet()
            ->SetCellValue('A' . $rowCount, 'Product Code');

        $objPHPExcel
            ->getActiveSheet()
            ->SetCellValue('B' . $rowCount, 'Available Product');
        $objPHPExcel
            ->getActiveSheet()
            ->SetCellValue('C' . $rowCount, 'Received QTY');

        $objPHPExcel
            ->getActiveSheet()
            ->SetCellValue('D' . $rowCount, 'Issued QTY');
        $objPHPExcel
            ->getActiveSheet()
            ->SetCellValue('E' . $rowCount, 'Station');
        $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, 'Status');
        $objPHPExcel
            ->getActiveSheet()
            ->SetCellValue('G' . $rowCount, 'Activity Date');
        ...
        foreach ($data as $value) {
            $objPHPExcel
                ->getActiveSheet()
                ->SetCellValue('A' . $rowCount, $value->code);
            $objPHPExcel
                ->getActiveSheet()
                ->setCellValueExplicit('B' . $rowCount, $value->available_qty);
            $objPHPExcel
                ->getActiveSheet()
                ->SetCellValue('C' . $rowCount, $value->received_qty);
            $objPHPExcel
                ->getActiveSheet()
                ->SetCellValue('D' . $rowCount, $value->issued_qty);
            $objPHPExcel
                ->getActiveSheet()
            ...
            $objPHPExcel
                ->getActiveSheet()
                ->SetCellValue('F' . $rowCount, $value->s_status);
            $objPHPExcel
                ->getActiveSheet()
                ->SetCellValue('G' . $rowCount, $value->activity_date);
