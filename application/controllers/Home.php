...
class Home extends AUTH_Controller {
	...
	public function index() {
		...
		$data['jml_kota'] = $this->M_confirmation->total_rows();
		...
		//	$data_posisi[$index]['value'] = $pegawai_by_posisi->jml;
			...
			//$data_posisi[$index]['label'] = $value->id;
			
			$index++;
		}

		$kota 				= $this->M_confirmation->select_all_inproduction();
		$index = 0;
		foreach ($kota as $value) {
		    $color = '#'.$rand[rand(0,15)].$rand[rand(0,15)].$rand[rand(0,15)].$rand[rand(0,15)].$rand[rand(0,15)].$rand[rand(0,15)];

			//$pegawai_by_kota = $this->M_employees->select_by_position($value->id);

			//$data_kota[$index]['value'] = $pegawai_by_kota->jml;
			$data_kota[$index]['color'] = $color;
			$data_kota[$index]['highlight'] = $color;
			//$data_kota[$index]['label'] = $value->id;
