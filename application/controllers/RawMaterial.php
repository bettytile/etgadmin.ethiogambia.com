...
class RawMaterial extends AUTH_Controller {
	...
	public function index() {
		...
		$data['dataRawmaterial'] = $this->M_rawmaterials->select_all();
		$data['dataProductType'] = $this->M_productType->select_all_productType();

		$data['page'] = "rawmaterial";
		$data['judul'] = "Raw Material Data";
		$data['deskripsi'] = "Manage Raw Material Data";

		$data['modal_insert_rawmaterial'] = show_my_modal('modals/modal_insert_rawmaterial', 'insert-rawmaterial', $data);

		$this->template->views('rawmaterial/home', $data);
	}

	public function show() {
		$data['dataRawmaterial'] = $this->M_rawmaterials->select_all();
		$this->load->view('rawmaterial/list_data', $data);
	}
	...
	public function addProduct() {
		...
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_rawmaterials->insert($data);
	...
	public function update() {
		...
		$data['dataRawmaterial'] = $this->M_rawmaterials->select_by_id($id);
		$data['dataProductType'] = $this->M_productType->select_all_productType();
		$data['userdata'] = $this->userdata;

		echo show_my_modal('modals/modal_update_rawmaterial', 'updates-rawmaterials', $data);
	}
	...
	public function prosesUpdate() {
		...
		$data = $this->input->post();
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_rawmaterials->update($data);
	...
	public function delete() {
		$id = $_POST['id'];
		$result = $this->M_rawmaterials->delete($id);
	...
	public function export() {
		...
		$data = $this->M_rawmaterials->select_all();
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
					if ($key != 1) {
						...
						$check = $this->M_preform->check_name($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_preform->insert_batch($resultData);
					if ($result > 0) {
